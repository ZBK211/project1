...
def pack(data: pd.DataFrame, comp_feat: Dict, prot_feat: Dict) -> pd.DataFrame:
    ...
    for i in range(len(data)):
        ...
        # 处理化合物特征（保持不变）
        comp_vec = comp_feat[str(cid)]
        
        # 处理蛋白质特征
        prot_data = prot_feat[pid]
        if isinstance(prot_data, dict):
            # 新格式：包含1D和3D特征
            prot_vec_1d = prot_data['1d']
            prot_vec_3d = prot_data.get('3d', np.zeros(256, dtype=np.float32))
            # 拼接1D和3D特征
            prot_vec = np.concatenate([prot_vec_1d, prot_vec_3d])
        else:
            # 旧格式：只有1D特征
            prot_vec = prot_data
        
        # 合并化合物和蛋白质特征
        combined_vec = list(comp_vec) + list(prot_vec)
        vecs.append(combined_vec)
